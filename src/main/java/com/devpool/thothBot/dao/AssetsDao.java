...
public class AssetsDao {
    ...
    public Optional<Asset> getAssetInformation(String policyId, String assetName) {
        List<Asset> assets = this.namedParameterJdbcTemplate.query("select id, policy_id, asset_name, asset_display_name, decimals from assets where policy_id = :policy_id and asset_name = :asset_name",
    ...
    public void addNewAsset(String policyId, String assetName, String assetDisplayName, Integer decimals) {

        // Check again for existance
        if (getAssetInformation(policyId, assetName).isEmpty()) {
            namedParameterJdbcTemplate.update("insert into assets (policy_id, asset_name, decimals, asset_display_name) values (:policy_id, :asset_name, :decimals, :asset_display_name)",
                    new MapSqlParameterSource(Map.of(FIELD_POLICY_ID, policyId, FIELD_ASSET_NAME, assetName, FIELD_ASSET_DISPLAY_NAME, assetDisplayName, FIELD_DECIMALS, decimals)));
